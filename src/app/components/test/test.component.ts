...
export class TestComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    this.canvas = this.el.nativeElement.querySelector('#js-canvas');
    ...
    this.image.src = "https://ftp.mathilde-ads.com/151-7e894c5ae49db3f905af1e9e4aaca8f0.webp";
    this.image.onload = () => {
      this.ctx.drawImage(this.image, 0, 0);
      this.el.nativeElement.querySelector('.form').style.visibility = 'visible';
    };
  ...
  getFilledInPixels(stride) {
    ...
    for (let i = 0; i < l; i += stride) {
      // Convert the number to a string before passing to parseInt
      if (parseInt(pdata[i].toString(), 10) === 0) {
        count++;
      }
    }
